...
if [[ "$STATUS" == "build_failure" ]]; then
  BUILD_LOG=$(tail -n 20 build/reports/build.log 2>/dev/null || echo "No build log found.")
  COLOR=15158332
  TITLE="❌ Build Failed"
  ...
  EXTRA_FIELDS="{\"name\": \"Error Log\", \"value\": \"\`\`\`$BUILD_LOG\`\`\`\", \"inline\": false},"
    ...
    TITLE="❌ Upload Failed"
    FOOTER="Failed Job via GitHub Actions"
    EXTRA_FIELDS="{\"name\": \"Error Log\", \"value\": \"\`$UPLOAD_ERROR\`\", \"inline\": false},"
    ...
    if [[ -n "$JAR_NAME" && -f "artifacts/$JAR_NAME" ]]; then
      ...
      if [[ "$JAR_SIZE" != "unknown" ]]; then
        if [ "$JAR_SIZE" -lt 1024 ]; then
          SIZE="\\\`$JAR_SIZE bytes\\\`"
        elif [ "$JAR_SIZE" -lt $((1024 * 1024)) ]; then
          SIZE="\\\`$(echo "$JAR_SIZE / 1024" | bc) KB\\\`"
        else
          SIZE="\\\`$(echo "scale=2; $JAR_SIZE / 1024 / 1024" | bc) MB\\\`"
        fi
        EXTRA_FIELDS="{\"name\": \"Jar & Size\", \"value\": \"\`$JAR_NAME\` ($SIZE)\", \"inline\": true},
        {\"name\": \"From → To\", \"value\": \"\`artifacts/\` → \`/plugins \`\", \"inline\": true},"
      fi
    fi
else
  COLOR=3066993
  ...
  if [[ "$JAR_SIZE" != "unknown" ]]; then
    ...
    EXTRA_FIELDS="{\"name\": \"Jar & Size\", \"value\": \"\`$JAR_NAME\` ($SIZE)\", \"inline\": true},
    {\"name\": \"Build Duration\", \"value\": \"\`$BUILD_DURATION s\`\", \"inline\": true},"
  fi
fi
